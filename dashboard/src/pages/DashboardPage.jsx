      ...
      <table className="w-full min-w-[630px] text-left text-xs">
        <thead className="border-y border-slate-100 bg-slate-50 text-slate-500">
          <tr>
            ...
            <th className="px-3 py-2.5 font-semibold">Egg ID</th>
            <th className="px-3 py-2.5 font-semibold">Weight (g)</th>
...
export default function DashboardPage() {
  ...
  const data = useMemo(() => {
    ...
    inspections.forEach((scan) => dailyCounts.set(scan.date, (dailyCounts.get(scan.date) || 0) + 1))
    const byDay = [...dailyCounts.entries()]
  ...
  return (
    <div>
      <PageHeader
        title="Dashboard"
        actions={
          <span className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard
          ...
          detail="Quality result: good"
          icon={CheckCircle2}
          tone="green"
        />
        <StatCard
          label="Defective Eggs"
          value={data.defective.toLocaleString()}
          detail="Quality result: defective"
          icon={AlertTriangle}
          tone="red"
        />
        <StatCard
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-12">
        <ChartCard title="Egg Size Distribution" className="xl:col-span-4">
          <div className="flex min-h-56 flex-col sm:flex-row sm:items-center">
            ...
            <div className="grid shrink-0 grid-cols-2 gap-x-3 gap-y-1 text-[11px] sm:block">
              {data.sizes.map((item) => (
                <div key={item.name} className="flex items-center gap-1.5 py-1 text-slate-600">
        ...
        <ChartCard title="Daily Egg Inspections" className="xl:col-span-4">
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={data.byDay} margin={{ top: 8, right: 0, left: -22, bottom: 0 }}>
        ...
        <ChartCard title="Quality Distribution" className="md:col-span-2 xl:col-span-4">
          <div className="relative flex min-h-56 items-center justify-center">
            ...
            <div className="pointer-events-none absolute text-center">
              ...
              <p className="text-[11px] text-slate-500">Inspections</p>
            </div>
          </div>
        </ChartCard>
      </div>
      <section className="mt-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-sm font-bold text-slate-800">Recent Scans</h2>
          <button
        ...
        <ScanTable rows={inspections.slice(0, 6)} />
      </section>
    </div>
  )
}
