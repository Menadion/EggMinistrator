...
import { ChevronLeft, ChevronRight, Database, Download, FileBarChart, FileText, Printer, RotateCcw, Save } from 'lucide-react'
...
const formatShortDate = (value) => new Intl.DateTimeFormat('en-US', { month: 'short', day: 'numeric', timeZone: 'UTC' }).format(new Date(`${value}T00:00:00Z`))
...
const groupDetails = (scan, groupBy) => {
  ...
  if (groupBy === 'Quality') return { key: scan.quality, label: classificationLabel(scan.quality), sort: qualities.indexOf(scan.quality) }
...
function PreviewPagination({ page, pageCount, total, pageSize, label, onPageChange }) {
  ...
  return <div className="mt-4 flex flex-col gap-3 text-sm text-slate-500 sm:flex-row sm:items-center sm:justify-between"><span>Showing {first.toLocaleString()}–{last.toLocaleString()} of {total.toLocaleString()} {label}</span><div className="flex items-center gap-2"><button disabled={page === 1} onClick={() => onPageChange(page - 1)} className="inline-flex items-center gap-1 rounded-md border border-slate-200 px-3 py-1.5 text-sm disabled:cursor-not-allowed disabled:opacity-40"><ChevronLeft size={15} />Previous</button><span className="text-xs font-medium">Page {page} of {pageCount}</span><button disabled={page === pageCount} onClick={() => onPageChange(page + 1)} className="inline-flex items-center gap-1 rounded-md border border-slate-200 px-3 py-1.5 text-sm disabled:cursor-not-allowed disabled:opacity-40">Next<ChevronRight size={15} /></button></div></div>
}
...
export default function ReportsPage() {
  ...
  const generateReport = () => { setReport({ ...builder, generatedAt: new Date() }); setGroupPage(1); setDetailPage(1) }
  const resetBuilder = () => { setBuilder(defaultBuilder); setReport(null); setGroupPage(1); setDetailPage(1) }
  ...
  const chartGroupBy = report?.groupBy === 'Day' && groups.length > GROUP_PAGE_SIZE ? 'Week' : report?.groupBy
  const chartGroups = useMemo(() => report ? buildGroups(reportScans, chartGroupBy) : [], [report, reportScans, chartGroupBy])
  const chartData = useMemo(() => chartGroups.map((group) => ({ ...group, chartLabel: chartGroupBy === 'Week' ? formatShortDate(group.key) : group.label })), [chartGroups, chartGroupBy])
  ...
  const showDetails = report?.reportType === 'Detailed Records' || Boolean(report?.includeDetails) || groups.length <= 1
  ...
  const visibleDetails = reportScans.slice((detailPage - 1) * DETAIL_PAGE_SIZE, detailPage * DETAIL_PAGE_SIZE)
  const chartTitle = report?.groupBy === 'Day' && chartGroupBy === 'Week' ? 'Weekly Inspection Volume Summary' : 'Inspection Volume'
  ...
  const keyFindingsSection = <section className="mt-7 border-t pt-6"><h2 className="mb-3 text-base font-bold">Key Findings</h2><ul className="space-y-2 text-sm text-slate-700">{findings.map((finding) => <li key={finding}>• {finding}</li>)}</ul></section>
    ...
    <div className="reports-page">
      ...
      <section className="reports-builder rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          ...
          <label><span className="mb-1.5 block text-xs font-semibold text-slate-600">Quality</span><select value={builder.quality} onChange={(event) => updateBuilder('quality', event.target.value)} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm">{qualities.map((quality) => <option key={quality}>{quality === 'All Quality' ? 'All Quality Results' : classificationLabel(quality)}</option>)}</select></label>
      ...
      {report && <section className="report-preview mt-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
        ...
        <header className="border-b-2 border-forest-800 pb-5"><div className="flex items-start justify-between gap-4"><div className="grid h-12 w-12 shrink-0 place-items-center overflow-hidden rounded-xl"><img src={eggministratorLogo} alt="Eggministrator logo" className="h-full w-full translate-y-2 scale-[2.5] object-cover" /></div><div className="text-right text-sm"><p className="text-slate-500">Generated At</p><p className="mt-1 font-semibold text-slate-900">{generatedAtLabel}</p></div></div><h1 className="mt-4 text-2xl font-bold text-slate-900">{report.reportType}</h1></header>
        ...
        {showsChart && <section className="mt-7"><h2 className="mb-1 text-base font-bold">{chartTitle}</h2>{chartGroupBy !== report.groupBy && <p className="mb-3 text-sm text-slate-500">This visual summary uses weekly totals to keep the long daily report readable.</p>}<ResponsiveContainer width="100%" height={260}><BarChart data={chartData}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="chartLabel" tick={{ fontSize: 11 }} interval="preserveStartEnd" /><YAxis /><Tooltip labelFormatter={(_, payload) => payload?.[0]?.payload?.label || 'Inspection period'} /><Bar dataKey="inspections" fill="#227849" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer></section>}
        {showsChart && keyFindingsSection}
        {showDetails && <section className="mt-7"><h2 className="mb-3 text-base font-bold">Detailed Inspection Records</h2><div className="overflow-x-auto"><table className="w-full min-w-[650px] text-sm"><thead className="border-y bg-slate-50 text-left text-xs text-slate-600"><tr><th className="px-3 py-2.5">Egg ID</th><th className="px-3 py-2.5">Date</th><th className="px-3 py-2.5">Time</th><th className="px-3 py-2.5">Weight</th><th className="px-3 py-2.5">Size</th><th className="px-3 py-2.5">Result</th></tr></thead><tbody>{visibleDetails.map((scan) => <tr key={scan.eggId} className="border-b"><td className="px-3 py-2.5 font-medium">{scan.eggId}</td><td className="px-3 py-2.5">{scan.displayDate || '—'}</td><td className="px-3 py-2.5">{scan.time || '—'}</td><td className="px-3 py-2.5">{weightLabel(scan.weight)}</td><td className="px-3 py-2.5">{scan.size || '—'}</td><td className="px-3 py-2.5">{classificationLabel(scan.quality)}</td></tr>)}{reportScans.length === 0 && <tr><td colSpan="6" className="px-3 py-8 text-center text-slate-500">No inspections match the selected criteria.</td></tr>}</tbody></table></div><PreviewPagination page={detailPage} pageCount={detailPageCount} total={reportScans.length} pageSize={DETAIL_PAGE_SIZE} label="detailed records" onPageChange={setDetailPage} /></section>}
        {showGrouped && <section className="mt-7"><h2 className="mb-3 text-base font-bold">Grouped Report</h2><div className="overflow-x-auto"><table className="w-full min-w-[700px] text-sm"><thead className="border-y bg-slate-50 text-left text-xs text-slate-600"><tr><th className="px-3 py-2.5">{report.groupBy}</th><th className="px-3 py-2.5 text-right">Inspections</th><th className="px-3 py-2.5 text-right">Good Eggs</th><th className="px-3 py-2.5 text-right">Defective Eggs</th><th className="px-3 py-2.5 text-right">Average Weight</th><th className="px-3 py-2.5 text-right">Defect Rate</th></tr></thead><tbody>{visibleGroups.map((group) => <tr key={group.key} className="border-b"><td className="px-3 py-2.5 font-medium">{group.label}</td><td className="px-3 py-2.5 text-right">{group.inspections}</td><td className="px-3 py-2.5 text-right">{group.good}</td><td className="px-3 py-2.5 text-right">{group.defective}</td><td className="px-3 py-2.5 text-right">{group.averageWeight === null ? 'Not available' : `${group.averageWeight.toFixed(1)} g`}</td><td className="px-3 py-2.5 text-right">{group.defectRate.toFixed(1)}%</td></tr>)}</tbody></table></div><PreviewPagination page={groupPage} pageCount={groupPageCount} total={groups.length} pageSize={GROUP_PAGE_SIZE} label={`${report.groupBy.toLowerCase()} groups`} onPageChange={setGroupPage} /></section>}
        {!showsChart && keyFindingsSection}
      </section>}
    </div>
  )
