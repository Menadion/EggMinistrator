  ...
  useEffect(() => {
    const loadHistory = async () => {
      ...
      try {
        ...
        setSelectedDate((current) => current || result.inspections?.[0]?.date || '')
      } catch (error) {
  ...
  return (
    <div>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm sm:p-4">
        <p className="mb-3 text-xs font-medium text-slate-500">Data source: MariaDB</p>
        ...
        <div className="grid gap-3 lg:grid-cols-[170px_minmax(0,1fr)_150px_150px]">
          ...
          <select value={quality} onChange={changeFilter(setQuality)} className="min-h-11 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-600"><option value="All Quality">All AI Results</option><option value="good">Good</option><option value="defective">Defective</option></select>
        </div>
        <div className="mt-5 overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="border-y border-slate-100 bg-slate-50 text-xs text-slate-500"><tr><th className="px-3 py-3 font-semibold">Date</th><th className="px-3 py-3 font-semibold">Time</th><th className="px-3 py-3 font-semibold">Egg ID</th><th className="px-3 py-3 font-semibold">Weight (g)</th><th className="px-3 py-3 font-semibold">Size</th><th className="px-3 py-3 font-semibold">AI Result</th><th className="px-3 py-3 font-semibold">Device</th></tr></thead><tbody>{isLoading && <tr><td colSpan="7" className="px-3 py-10 text-center text-slate-500">Loading MariaDB records...</td></tr>}{!isLoading && visible.map((scan) => <tr key={scan.eggId} className="border-b border-slate-100 text-slate-600"><td className="px-3 py-3">{scan.displayDate}</td><td className="px-3 py-3">{scan.time}</td><td className="px-3 py-3 font-semibold text-slate-800">{scan.eggId}</td><td className="px-3 py-3">{scan.weight ?? '—'}</td><td className="px-3 py-3"><SizeBadge size={scan.size} /></td><td className="px-3 py-3"><QualityBadge quality={scan.quality} /></td><td className="px-3 py-3">{scan.device}</td></tr>)}{!isLoading && visible.length === 0 && <tr><td colSpan="7" className="px-3 py-10 text-center text-slate-500">No inspections match your filters.</td></tr>}</tbody></table></div>
