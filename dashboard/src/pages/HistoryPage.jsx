  ...
  const applyOverride = async (scan, label) => {
    ...
    try {
      const response = await authenticatedFetch(
        `/api/inspections/${encodeURIComponent(scan.inspectionCode)}/override`,
        {
          method: 'PATCH',
      ...
      setHistoryScans((scans) =>
        scans.map((item) =>
          item.inspectionCode === scan.inspectionCode
            ? { ...item, quality: label, isOverridden: true }
            : item
        )
      )
    } catch (error) {
    ...
    () =>
      historyScans.filter(
        (scan) =>
          ...
          (size === 'All Sizes' || scan.size === size) &&
          (quality === 'All Quality' || scan.quality === quality) &&
          Object.values(scan).join(' ').toLowerCase().includes(search.toLowerCase())
      ),
    [historyScans, search, size, quality, selectedDate]
  )
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm sm:p-4">
        ...
        <div className="grid gap-3 lg:grid-cols-[170px_minmax(0,1fr)_150px_150px]">
          <input
            ...
            className="min-h-11 rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-600"
          />
          <SearchInput
            value={search}
            onChange={(event) => {
            ...
            placeholder="Search Egg ID or device"
          />
          <select
            value={size}
            onChange={changeFilter(setSize)}
          ...
          <select
            value={quality}
            onChange={changeFilter(setQuality)}
            className="min-h-11 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-600"
          >
            <option value="All Quality">All Quality Results</option>
              ...
              {!isLoading &&
                visible.map((scan) => (
                  <tr key={scan.eggId} className="border-b border-slate-100 text-slate-600">
                    ...
                    <td className="px-3 py-3">
                      <QualityBadge quality={scan.quality} />
                    </td>
                    <td className="px-3 py-3">{scan.device}</td>
                    <td className="px-3 py-3">
                      <select
                        value={scan.quality || ''}
                        disabled={overridingCode === scan.inspectionCode}
                        onChange={(event) => applyOverride(scan, event.target.value)}
                        aria-label={`Override result for ${scan.eggId}`}
                        className="min-h-9 rounded-lg border border-slate-200 bg-white px-2 py-1 text-xs text-slate-600 disabled:opacity-40"
                      >
                        <option value="good">Good</option>
                        <option value="defective">Defective</option>
                        <option value="not_an_egg">Not an Egg</option>
                      </select>
                      {scan.isOverridden && (
                        <span
                          title={`AI said: ${scan.aiQuality || 'unknown'}`}
                          className="ml-2 rounded-full bg-amber-50 px-2 py-0.5 text-[10px] font-semibold text-amber-800"
                        >
                          overridden
                        </span>
                      )}
        ...
        <div className="mt-4 flex flex-col gap-3 text-sm text-slate-500 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-1">
            ...
            {pages.map((item) =>
              typeof item === 'string' ? (
                <span key={item} className="grid h-8 w-6 place-items-center text-slate-400">
                  …
                </span>
              ) : (
                <button
                  ...
                  className={`h-8 w-8 rounded-md text-xs font-semibold ${page === item ? 'bg-forest-800 text-white' : 'hover:bg-slate-100'}`}
                >
                  {item}
                </button>
              )
