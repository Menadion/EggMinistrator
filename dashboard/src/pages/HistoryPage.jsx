      ...
      <section className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm sm:p-4">
        ...
        <div className="mt-5 overflow-x-auto">
          <table className="w-full min-w-[760px] text-left text-sm">
            ...
            <tbody>
              {isLoading && <tr><td colSpan="8" className="px-3 py-10 text-center text-slate-500">Loading MariaDB records...</td></tr>}
              {!isLoading && visible.map((scan) => (
                <tr key={scan.eggId} className="border-b border-slate-100 text-slate-600">
                  ...
                  <td className="px-3 py-3">
                    <select value={scan.quality || ''} disabled={overridingCode === scan.inspectionCode} onChange={(event) => applyOverride(scan, event.target.value)} aria-label={`Override result for ${scan.eggId}`} className="min-h-9 rounded-lg border border-slate-200 bg-white px-2 py-1 text-xs text-slate-600 disabled:opacity-40">
                      <option value="good">Good</option>
                      <option value="defective">Defective</option>
                      <option value="not_an_egg">Not an Egg</option>
                    </select>
                    {scan.isOverridden && <span title={`AI said: ${scan.aiQuality || 'unknown'}`} className="ml-2 rounded-full bg-amber-50 px-2 py-0.5 text-[10px] font-semibold text-amber-800">overridden</span>}
                  </td>
                </tr>
              ))}
              {!isLoading && visible.length === 0 && <tr><td colSpan="8" className="px-3 py-10 text-center text-slate-500">No inspections match your filters.</td></tr>}
            </tbody>
          </table>
        </div>
