  ...
  return (
    <main className="grid min-h-screen bg-cream-100 lg:grid-cols-2">
      <section className="hidden bg-gradient-to-br from-forest-950 via-forest-900 to-forest-700 p-12 text-white lg:flex lg:flex-col lg:justify-between">
        <div className="flex items-center gap-3 text-xl font-bold"><span className="grid h-11 w-11 place-items-center rounded-xl bg-cream-50 text-amber-500"><Egg size={28} /></span>EggMinistrator</div>
        <div className="max-w-lg"><p className="mb-5 inline-flex items-center gap-2 rounded-full border border-green-500/40 bg-green-800/50 px-3 py-1 text-sm text-green-100"><ShieldCheck size={16} />Smart egg inspection management</p><h1 className="text-5xl font-bold leading-tight">Quality insights for every egg.</h1><p className="mt-5 text-lg leading-8 text-green-100">Monitor grades, AI quality checks, and production trends from one simple dashboard.</p></div>
      ...
      <section className="flex items-center justify-center p-5 sm:p-10">
        <form onSubmit={submit} className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-xl shadow-green-950/8 sm:p-8">
          <div className="mb-8 lg:hidden"><div className="flex items-center gap-2 text-xl font-bold text-forest-900"><span className="grid h-9 w-9 place-items-center rounded-lg bg-green-50 text-amber-500"><Egg size={23} /></span>EggMinistrator</div></div>
          ...
          <p className="mt-5 rounded-lg bg-amber-50 p-3 text-center text-xs text-amber-800">Demo Mode: any credentials open the sample dashboard.</p>
        </form>
      </section>
    </main>
  )
