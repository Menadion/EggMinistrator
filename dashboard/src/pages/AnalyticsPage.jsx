...
export default function AnalyticsPage() {
  return (
    <div>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-3"><ChartCard title="Spoilage Percentage Over Time"><TrendChart data={spoilageTrend} type="area" color="#ef5350" /></ChartCard><ChartCard title="Size Distribution"><ResponsiveContainer width="100%" height={240}><PieChart><Pie data={sizeDistribution} dataKey="value" nameKey="name" outerRadius={90} innerRadius={45}>{sizeDistribution.map((item) => <Cell key={item.name} fill={item.color} />)}</Pie><Tooltip /><Legend verticalAlign="bottom" align="center" iconType="circle" wrapperStyle={{ fontSize: '12px', paddingTop: '8px' }} /></PieChart></ResponsiveContainer></ChartCard><ChartCard title="Good vs Spoiled"><div className="relative"><ResponsiveContainer width="100%" height={240}><PieChart><Pie data={qualityDistribution} dataKey="value" nameKey="name" outerRadius={90} innerRadius={58}>{qualityDistribution.map((item) => <Cell key={item.name} fill={item.color} />)}</Pie><Tooltip position={{ x: 12, y: 12 }} cursor={false} formatter={(value) => [`${value} eggs`, 'Count']}/><Legend verticalAlign="bottom" align="center" iconType="circle" wrapperStyle={{ fontSize: '12px', paddingTop: '8px' }} /></PieChart></ResponsiveContainer><div className="pointer-events-none absolute inset-0 grid place-items-center text-center"><div><p className="text-xl font-bold">87.1%</p><p className="text-xs text-slate-500">Good eggs</p></div></div></div></ChartCard></div>
    </div>
  )
}
