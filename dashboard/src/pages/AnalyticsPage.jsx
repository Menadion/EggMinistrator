...
export default function AnalyticsPage() {
  ...
  const { inspections, isLoading, error: databaseError, refresh: refreshDatabase } = useDatabaseInspections()
  ...
  useEffect(() => {
    ...
    setFilters((current) => ({ ...current, startDate: dates[0], endDate: dates.at(-1) }))
    setPreset('custom')
  }, [inspections])
  ...
  const defectData = useMemo(() => aggregateScans(selectedScans, 'daily').map((point) => {
    ...
    const eggScans = scans.filter((scan) => scan.quality !== 'not_an_egg')
    const defective = eggScans.filter((scan) => scan.quality === 'defective').length
    return { ...point, rate: eggScans.length ? Number(((defective / eggScans.length) * 100).toFixed(1)) : 0 }
  }), [selectedScans])
  ...
  const presetClass = (name) => `min-h-11 rounded-full px-3 py-1.5 text-xs font-semibold ${preset === name ? 'bg-forest-800 text-white' : 'border border-slate-200 bg-white text-slate-600 hover:bg-slate-50'}`
  ...
  return (
    <div>
      <PageHeader title="Analytics" description={`${isLoading ? 'Loading' : selectedScans.length.toLocaleString()} MariaDB inspections selected • Refreshed ${refreshedLabel}`} />
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm sm:p-4">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-6"><label><span className="mb-1 block text-xs font-medium text-slate-500">Start date</span><input type="date" value={filters.startDate} onChange={(event) => updateFilter('startDate', event.target.value)} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" /></label><label><span className="mb-1 block text-xs font-medium text-slate-500">End date</span><input type="date" value={filters.endDate} onChange={(event) => updateFilter('endDate', event.target.value)} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" /></label><label><span className="mb-1 block text-xs font-medium text-slate-500">Aggregation</span><select value={aggregation} onChange={(event) => setAggregation(event.target.value)} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm"><option value="daily">Daily</option><option value="weekly">Weekly</option><option value="monthly">Monthly</option></select></label><label><span className="mb-1 block text-xs font-medium text-slate-500">Egg size</span><select value={filters.size} onChange={(event) => updateFilter('size', event.target.value)} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm">{sizes.map((size) => <option key={size}>{size}</option>)}</select></label><label><span className="mb-1 block text-xs font-medium text-slate-500">AI result</span><select value={filters.quality} onChange={(event) => updateFilter('quality', event.target.value)} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm">{qualities.map((quality) => <option key={quality}>{quality === 'All Quality' ? 'All AI Results' : qualityLabel(quality)}</option>)}</select></label><label><span className="mb-1 block text-xs font-medium text-slate-500">Device</span><select disabled className="w-full cursor-not-allowed rounded-lg border border-slate-200 bg-slate-100 px-3 py-2 text-sm text-slate-400"><option>Not Configured</option></select></label></div>
